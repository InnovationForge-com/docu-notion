...
async function processImageBlock(
  ...
  if (canDetermineFilenameEarly) {
    ...
    if (imageSet.primaryFileOutputPath && fs.existsSync(imageSet.primaryFileOutputPath)) {
      ...
      await saveLocalizedImages(imageSet);
    } else {
...
async function saveLocalizedImages(imageSet: ImageSet): Promise<void> {
